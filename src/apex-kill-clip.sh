...
while read row; do
    ...
    if [ "$this_src_file" != "$last_src_file" ] || [ "$this_src_file" = "$last_src_file" -a $((this_unix_time-last_unix_time)) -gt $battle_duration ]; then
        ...
        if [ `find ${export_csv_local_dir} -name "${this_unix_time}_*" | wc -l` -eq 0 -a `find ${export_csv_local_dir} -name "*_${this_basefilename}.mp4" | wc -l` -eq 0 -a ! -f ${this_save_file} ]; then
